...
To manufacture within a specification, the difference between the USL and the LSL must be greater than the total process variation, and the distribution should be well centered between the limits. One way to check this is the $C_\mathrm{p}$/$C_\mathrm{pk}$ analysis [@Montgomery_SQC]. Given a parameter distribution $x_i$ from a sample of size $N$, the arithmetic mean and the standard deviation are
