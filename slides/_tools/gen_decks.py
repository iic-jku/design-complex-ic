#!/usr/bin/env python3
# -------------------------------------------------
# Generate one reveal.js deck per chapter of dcic.qmd
# -------------------------------------------------
...
#   slides/<topic>.qmd             one deck per chapter
#   slides/index.qmd               overview page linking all decks
#   slides/_tools/numbers.json     book numbering of every sec/fig/eq/tbl/nte label
#
# Usage:
...
def book_includes(root=ROOT):
    """Top-level include paths of dcic.qmd, in order (project-relative)."""
    paths = []
...
def chapters(paths, root=ROOT):
    ...
    for path in paths:
        ...
        out.append({"topic": m.group(1), "file": path, "number": number,
                    "title": title, "deck": f"{m.group(1)}.qmd"})
    return out


DECK_TEMPLATE = """\
---
# Generated by slides/_tools/gen_decks.py from {book} -- do not edit.
title: "{title}"
...
format:
  revealjs:
    ...
    footer: "[DCIC lecture notes](../dcic.html#{sec_id})"
    menu:
      numbers: true
filters:
  - at: pre-ast
...
def render_files(root=ROOT):
    ...
    files["slides/index.qmd"] = OVERVIEW_TEMPLATE.format(book=BOOK, items=items)
